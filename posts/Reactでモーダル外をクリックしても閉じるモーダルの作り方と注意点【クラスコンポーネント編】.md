...
Reactでモーダルウィンドウを作る時に、「閉じるボタンだけじゃなくて、**モーダル外をクリックしても閉じる**ようにしたい」って思いませんか？

説明している記事が意外と少なかったので、自分で書いてみました(*´ω｀)
（探し方が下手なだけかなｗ）

アンマウント時にエラーを起こしやすいので、注意点も一緒に書いておきました。
（アンマウントコンポーネントでステート更新されてるみたいなエラーが出る。）
